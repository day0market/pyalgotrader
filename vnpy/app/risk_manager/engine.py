...
class RiskManagerEngine(BaseEngine):
    ...
    def update_setting(self, setting: dict):
        ...
        if self.active:
            self.write_log(" transaction risk control function is activated ")
        else:
            self.write_log(" transaction risk control function stop ")
    ...
    def check_risk(self, req: OrderRequest, gateway_name: str):
        ...
        # Check order volume
        if req.volume <= 0:
            self.write_log(" it must be greater than the number of commission 0")
            return False

        if req.volume > self.order_size_limit:
            self.write_log(
                f" number of single delegate {req.volume}， exceed the limit {self.order_size_limit}")
            return False

        # Check trade volume
        if self.trade_count >= self.trade_limit:
            self.write_log(
                f" the total number of contracts traded today {self.trade_count}， exceed the limit {self.trade_limit}")
            return False

        # Check flow count
        if self.order_flow_count >= self.order_flow_limit:
            self.write_log(
                f" commissioned by the number of streams {self.order_flow_count}， each exceeds the limit {self.order_flow_clear} second {self.order_flow_limit} secondary ")
            return False
        ...
        if active_order_count >= self.active_order_limit:
            self.write_log(
                f" the number of currently active delegate {active_order_count}， exceed the limit {self.active_order_limit}")
            return False
        ...
        if req.symbol in self.order_cancel_counts and self.order_cancel_counts[req.symbol] >= self.order_cancel_limit:
            self.write_log(
                f" day {req.symbol} the number of withdrawals {self.order_cancel_counts[req.symbol]}， exceed the limit {self.order_cancel_limit}")
            return False
