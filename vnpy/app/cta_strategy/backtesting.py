...
class BacktestingEngine:
    ...
    def load_data(self):
        ...
        if self.start >= self.end:
            self.output(" start date must be less than the end date ")
            return
    ...
    def run_backtesting(self):
        ...
        self.output(" strategy initialization is complete ")
        ...
        self.output(" historical data playback end ")

    def calculate_result(self):
        """"""
        self.output(" i began to calculate mark-to-market gains and losses ")

        if not self.trades:
            self.output(" transaction records is empty ， unable to calculate ")
            return
        ...
        self.output(" mark-to-market profit and loss calculation is completed ")
        return self.daily_df

    def calculate_statistics(self, df: DataFrame = None, output=True):
        """"""
        self.output(" began to calculate statistical indicators strategy ")
        ...
        if output:
            self.output("-" * 30)
            self.output(f" the first trading day ：\t{start_date}")
            self.output(f" last trading day ：\t{end_date}")

            self.output(f" total trading day ：\t{total_days}")
            self.output(f" profit trading day ：\t{profit_days}")
            self.output(f" loss trading day ：\t{loss_days}")

            self.output(f" seed money ：\t{self.capital:,.2f}")
            self.output(f" end funds ：\t{end_balance:,.2f}")

            self.output(f" the total rate of return ：\t{total_return:,.2f}%")
            self.output(f" annualized earnings ：\t{annual_return:,.2f}%")
            self.output(f" the maximum retracement : \t{max_drawdown:,.2f}")
            self.output(f" the maximum percentage retracement : {max_ddpercent:,.2f}%")

            self.output(f" total profit and loss ：\t{total_net_pnl:,.2f}")
            self.output(f" total fee ：\t{total_commission:,.2f}")
            self.output(f" the total slippage ：\t{total_slippage:,.2f}")
            self.output(f" the total turnover ：\t{total_turnover:,.2f}")
            self.output(f" the total transaction amount ：\t{total_trade_count}")

            self.output(f" average daily profit and loss ：\t{daily_net_pnl:,.2f}")
            self.output(f" average daily fee ：\t{daily_commission:,.2f}")
            self.output(f" average daily slippage ：\t{daily_slippage:,.2f}")
            self.output(f" average daily turnover ：\t{daily_turnover:,.2f}")
            self.output(f" average daily transaction amount ：\t{daily_trade_count}")

            self.output(f" average daily rate of return ：\t{daily_return:,.2f}%")
            self.output(f" return standard deviation ：\t{return_std:,.2f}%")
            self.output(f"Sharpe Ratio：\t{sharpe_ratio:,.2f}")
            self.output(f" earnings ratio retracement ：\t{return_drawdown_ratio:,.2f}")
