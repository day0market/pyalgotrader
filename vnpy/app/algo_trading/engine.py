...
class AlgoEngine(BaseEngine):
    ...
    def init_engine(self):
        """"""
        self.write_log(" algorithmic trading engine start ")
    ...
    def subscribe(self, algo: AlgoTemplate, vt_symbol: str):
        ...
        if not contract:
            self.write_log(f' subscribe to market failure ， can not find contract ：{vt_symbol}', algo)
            return
    ...
    def send_order(
        ...
        contract = self.main_engine.get_contract(vt_symbol)
        if not contract:
            self.write_log(f' under orders fail ， can not find contract ：{vt_symbol}', algo)
            return
    ...
    def cancel_order(self, algo: AlgoTemplate, vt_orderid: str):
        ...
        if not order:
            self.write_log(f" commissioned cancellation failure ， no proxy ：{vt_orderid}", algo)
            return
    ...
    def get_tick(self, algo: AlgoTemplate, vt_symbol: str):
        ...
        if not tick:
            self.write_log(f" quote failure ， can not find quotes ：{vt_symbol}", algo)
    ...
    def get_contract(self, algo: AlgoTemplate, vt_symbol: str):
        ...
        if not contract:
            self.write_log(f" queries contract failure ， can not find contract ：{vt_symbol}", algo)
