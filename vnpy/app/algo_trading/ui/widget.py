...
class AlgoWidget(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        start_algo_button = QtWidgets.QPushButton(" start algorithm ")
        start_algo_button.clicked.connect(self.start_algo)
        form.addRow(start_algo_button)

        load_csv_button = QtWidgets.QPushButton("CSV start up ")
        ...
        form.addRow(" configuration name ", self.setting_name_line)

        save_setting_button = QtWidgets.QPushButton(" save configuration ")
    ...
    def load_csv(self):
        ...
        path, type_ = QtWidgets.QFileDialog.getOpenFileName(
            self,
            u" loading algorithm configuration ",
            "",
            "CSV(*.csv)"
        )
        ...
        for field_name in self.widgets.keys():
            if field_name not in reader.fieldnames:
                QtWidgets.QMessageBox.warning(
                    self,
                    " field is missing ",
                    f"CSV file deletion algorithm {self.template_name} required fields {field_name}"
                )
                return
                ...
                if field_type == list:
                    ...
                    try:
                        ...
                        QtWidgets.QMessageBox.warning(
                            self,
                            " parameter error ",
                            f"{field_name} parameter type should be {field_type}， please check ！"
                        )
                        return
...
class AlgoManager(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        stop_all_button = QtWidgets.QPushButton(" full stop ")
        ...
        tab1.addTab(active_algo_monitor, " execution ")
        tab1.addTab(inactive_algo_monitor, " over ")

        log_monitor = LogMonitor(self.event_engine)
        tab2 = QtWidgets.QTabWidget()
        tab2.addTab(log_monitor, " journal ")
        ...
        tab3.addTab(setting_monitor, " configuration ")
