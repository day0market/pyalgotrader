...
class SpreadDataEngine:
    ...
    def start(self):
        ...
        self.write_log(" data engine start spread success ")
    ...
    def add_spread(
        ...
        if name in self.spreads:
            self.write_log(" spread creation failed ， name repeat ：{}".format(name))
            return
        ...
        self.write_log(" spread successfully created ：{}".format(name))
        self.put_data_event(spread)
    ...
    def remove_spread(self, name: str) -> None:
        ...
        self.save_setting()
        self.write_log(" spread successfully removed ：{}， after the restart to take effect ".format(name))
...
class SpreadAlgoEngine:
    ...
    def start(self):
        ...
        self.write_log(" spread algorithm engine starts successfully ")
    ...
    def start_algo(
        ...
        if not spread:
            self.write_log(" creating spread algorithm fails ， can not find spread ：{}".format(spread_name))
            return ""
    ...
    def stop_algo(
        ...
        if not algo:
            self.write_log(" algorithm fails to stop the spread ， can not find algorithm ：{}".format(algoid))
            return
    ...
    def cancel_order(self, algo: SpreadAlgoTemplate, vt_orderid: str) -> None:
        ...
        if not order:
            self.write_algo_log(algo, " withdrawals failure ， no proxy {}".format(vt_orderid))
            return
    ...
    def cancel_order(self, strategy: SpreadStrategyTemplate, vt_orderid: str):
        ...
        if not order:
            self.write_strategy_log(
                strategy, " withdrawals failure ， no proxy {}".format(vt_orderid))
            return
