    ...
    def wrapper(func):
        ...
        def wrapped(self, info, error_info, *args):
            ...
            if error_code != 0:
                ...
                msg = f' in  {function_name}  receive an error ({error_code})：{error_msg}'
                if write_log:
...
class ToraTdSpi(CTORATstpTraderSpi):
    ...
    def OnErrRtnOrderInsert(self, info: CTORATstpInputOrderField,
        ...
        self.gateway.on_order(order_data)
        self.gateway.write_log(f" single refused ({order_data.orderid}):"
                               f" error code :{error_info.ErrorID},  wrong information :{error_info.ErrorMsg}")
    ...
    def OnRspOrderInsert(self, info: CTORATstpInputOrderField,
        ...
        try:
            ...
            self.gateway.write_log(f" receive a single receipt unrecognized ({info.OrderRef})")
            return
        self.gateway.on_order(order_data)
    ...
    def OnRspQryPosition(self, info: CTORATstpPositionField, error_info: CTORATstpRspInfoField,
        ...
        if info.InvestorID != self.session_info.investor_id:
            self.gateway.write_log("OnRspQryPosition: receive position information from other accounts ")
            return
        if info.ExchangeID not in EXCHANGE_TORA2VT:
            self.gateway.write_log(
                f"OnRspQryPosition: ignores the unsupported exchange ：{info.ExchangeID}")
            return
    ...
    def OnFrontDisconnected(self, error_code: int) -> None:
        """"""
        self.gateway.write_log(
            f" transaction server is disconnected ({error_code}):{get_error_msg(error_code)}")
...
class ToraTdApi:
    ...
    def _if_error_write_log(self, error_code: int, function_name: str):
        ...
        if error_code != 0:
            ...
            msg = f' in execution  {function_name}  when an error occurs ({error_code}): {error_msg}'
            self.gateway.write_log(msg)
            return True
    ...
    def send_order(self, req: OrderRequest):
        ...
        # info.MinVolume = 0  #  when the volume type is MV valid 
        info.ForceCloseReason = TORA_TSTP_FCC_NotForceClose

        # info.IsSwapOrder = 0  #  it does not support single swap 
        # info.UserForceClose = 0  #  strong user comment mark 
        info.Operway = TORA_TSTP_OPERW_PCClient  #  on commission ：PC end commission 
    ...
    def cancel_order(self, req: CancelRequest):
        ...
        info.InvestorID = self.session_info.investor_id
        #  if not ：(16608)：VIP: unknown exchange symbol 
        info.ExchangeID = EXCHANGE_VT2TORA[req.exchange]
        info.SecurityID = req.symbol
        ...
        info.ActionFlag = TORA_TSTP_AF_Delete  # (12673)：VIP: cancellation information does not match with the original declaration 
        # info.ActionFlag = TORA_TSTP_AF_ForceDelete  # (12368)：VIP: abnormal state declaration 
