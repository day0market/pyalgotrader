...
class ToraMdSpi(CTORATstpMdSpi):
    ...
    def OnFrontDisconnected(self, error_code: int) -> Any:
        ...
        self.gateway.write_log(
            f" disconnect the server market ({error_code}):{get_error_msg(error_code)}")
    ...
    def OnRspError(
        ...
        self.gateway.write_log(f" quotes service receive an error message ({error_id})：{error_msg}")
    ...
    def OnRspUserLogin(
        ...
        if error_id != 0:
            error_msg = error_info.ErrorMsg
            self.gateway.write_log(f" quotes service logon failure ({error_id})：{error_msg}")
            return
    ...
    def OnRspUserLogout(
        ...
        if error_id != 0:
            error_msg = error_info.ErrorMsg
            self.gateway.write_log(f" quotes service failed logout ({error_id})：{error_msg}")
            return
        self.gateway.write_log(" logout server market success ")
