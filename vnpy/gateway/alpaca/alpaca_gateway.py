...
class AlpacaRestApi(RestClient):
    ...
    def on_cancel_order(self, data, request):
        """"""
        req = request.extra
        msg = f" withdrawals success ， no. commission ：{req.orderid}"
        self.gateway.write_log(msg)
    ...
    def on_packet(self, packet: dict):
        ...
        if stream == "authorization":
            ...
            if "trade_updates" in streams:
                self.gateway.write_log(" principal transactions push subscription success ")

            if "account_updates" in streams:
                self.gateway.write_log(" changes in funding push subscription success ")
...
class AlpacaDataRestApi(RestClient):
    ...
    def connect(
        ...
        self.gateway.write_log(" price REST API successful start ")
