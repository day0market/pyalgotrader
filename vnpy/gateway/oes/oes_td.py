...
class OesTdMessageLoop:
    ...
    def reconnect(self):
        """"""
        self.gateway.write_log(_(" attempting to reconnect to the transaction server 。"))
        ...
        while self._alive:
            ...
            if ret < 0:
                ...
                if is_disconnected(ret):
                    self.gateway.write_log(_(" connection with the transaction server has been disconnected 。"))
    ...
    def on_order_rejected(self, d: OesRspMsgBodyT):
        ...
        if not data.origClSeqNo:
            ...
            self.gateway.write_log(f" withdrawals failure ， order number ： {data.origClSeqNo}。 the reason ：{error_string}")
...
class OesTdApi:
    ...
    def connect(self):
        ...
        if not self._connect_ord_channel():
            self.gateway.write_log(_(" unable to initialize the transaction under a single channel (td_ord_server)"))

        if not self._connect_qry_channel():
            self.gateway.write_log(_(" unable to initialize the transaction query channel (td_qry_server)"))
    ...
    def _reconnect_ord_channel(self):
        with self._ord_reconnect_lock:  # prevent spawning multiple reconnect thread
            self.gateway.write_log(_(" reconnecting to deal single-channel "))
            while not self._connect_ord_channel():
                time.sleep(1)

            self.gateway.write_log(_(" successfully reconnected to the transactions under a single channel "))
    ...
    def send_order(self, vt_req: OrderRequest):
        ...
        if ret >= 0:
            ...
            self.gateway.write_log(_(" single failure "))  # todo: can I stringify error?
            if is_disconnected(ret):
                self.gateway.write_log(_(" when connecting single disconnected found ， we are trying to reconnect "))
    ...
    def cancel_order(self, vt_req: CancelRequest):
        ...
        ret = OesApi_SendOrderCancelReq(self._env.ordChannel,
                                        oes_req)
        if ret < 0:
            self.gateway.write_log(_(" withdrawals failure "))  # todo: can I stringify error?
            if is_disconnected(ret):  # is here any other ret code indicating connection lost?
                self.gateway.write_log(_(" connection found disconnected when withdrawals ， we are trying to reconnect "))
                self._schedule_reconnect_ord_channel()
