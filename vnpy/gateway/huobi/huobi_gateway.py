"""
 fire currency transaction interface 
"""
...
class HuobiRestApi(RestClient):
    ...
    def on_query_account(self, data, request):
        ...
        for d in data["data"]:
            if d["type"] == "spot":
                ...
                self.gateway.write_log(f" account code {self.account_id} search successful ")

        self.query_account_balance()

    def on_query_account_balance(self, data, request):
        """"""
        if self.check_error(data, " query account funds "):
            return
    ...
    def on_query_order(self, data, request):
        """"""
        if self.check_error(data, " inquiry commission "):
            return
    ...
    def on_cancel_order(self, data, request):
        ...
        if self.check_error(data, " withdrawals "):
            order.status = Status.REJECTED
        else:
            order.status = Status.CANCELLED
            self.gateway.write_log(f" successfully commissioned cancellation ：{order.orderid}")
