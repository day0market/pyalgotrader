...
class TapGateway(BaseGateway):
    ...
    def _async_callback_exception_handler(self, e: AsyncDispatchException):
        """"""
        error_str = (f" an internal error occurred ， position ：\n{e.instance}.{e.function_name}， details ：\n"
                     f"{e.what}\n"
                     )
        print(error_str)
...
class QuoteApi(ITapQuoteAPINotify):
    ...
    def OnRspLogin(self, errorCode: int, info: TapAPIQuotLoginRspInfo):
        ...
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" login failed server market ：{error_to_str(errorCode)}")
        else:
    ...
    def OnDisconnect(self, reasonCode: int):
        ...
        """
        self.gateway.write_log(f" disconnect the server market ， the reason ：{reasonCode}")
    ...
    def OnRspSubscribeQuote(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" subscribe to market failure ：{error_to_str(errorCode)}")
        else:
    ...
    def update_tick(self, info: TapAPIQuoteWhole):
        ...
        if not contract_info:
            self.gateway.write_log(f" quotes contract information could not be matched ：{symbol} with {exchange}")
            return
    ...
    def connect(self, username: str, password: str, host: str, port: int, auth_code: str):
        ...
        if not self.api:
            self.gateway.write_log(" price API initialization failed ")
            return
    ...
    def subscribe(self, req: SubscribeRequest):
        ...
        if not contract_info:
            self.gateway.write_log(
                f" can not find matching contracts ：{req.symbol} with {req.exchange.value}")
            return
...
class TradeApi(ITapTradeAPINotify):
    ...
    def OnRspLogin(self, errorCode: int, info: TapAPITradeLoginRspInfo):
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" login failed transaction server ：{error_to_str(errorCode)}")
        else:
    ...
    def OnRspQryCommodity(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(" query trading products information failed ")
            return
        ...
        if isLast == "Y":
            self.gateway.write_log(" query successful trading products information ")
    ...
    def OnRspQryContract(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(" query failed transaction contract information ")
            return
        ...
        if isLast == "Y":
            self.gateway.write_log(" query successful transaction contract information ")
            self.query_account()
    ...
    def OnRspQryAccount(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" query account information failed ")
            return
    ...
    def OnRspQryFund(
        ...
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" query failed financial information ")
            return

        self.update_account(info)

        if isLast == "Y":
            self.gateway.write_log(" query successful fund information ")
            self.query_position()
    ...
    def OnRspQryPositionSummary(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" query position information failed ")
            return
        ...
        if isLast == "Y":
            self.gateway.write_log(f" position information query success ")
            self.query_order()
    ...
    def OnRspQryOrder(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" information inquiry commission failed ")
            return
        ...
        if isLast == "Y":
            self.gateway.write_log(f" information inquiry commissioned successfully ")
            self.query_trade()
    ...
    def OnRtnOrder(self, info: TapAPIOrderInfoNotice):
        ...
        if info.ErrorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" under orders fail ：{error_to_str(info.ErrorCode)}")
    ...
    def OnRspQryFill(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" query failed transaction information ")
            return
        ...
        if isLast == "Y":
            self.gateway.write_log(f" query successful transaction information ")
    ...
    def OnRspOrderAction(
        ...
        """
        if errorCode != TAPIERROR_SUCCEED:
            self.gateway.write_log(f" entrusted operation failed ：{error_to_str(errorCode)}")
            return
    ...
    def connect(self, username: str, password: str, host: str, port: int, auth_code: str):
        ...
        if not self.api:
            self.gateway.write_log(" transaction API initialization failed ")
            return
    ...
    def send_order(self, req: OrderRequest):
        ...
        contract_info = contract_infos.get((req.symbol, req.exchange), None)
        if not contract_info:
            self.write_log(f" can not find matching contracts ：{req.symbol} with {req.exchange.value}")
            return ""

        if req.type not in ORDERTYPE_VT2TAP:
            self.write_log(f" it does not support the delegate type : {req.type.value}")
            return ""
