"""
 fire currency contracts interface 
"""
...
class HbdmRestApi(RestClient):
    ...
    def query_history(self, req: HistoryRequest):
        ...
        if resp.status_code // 100 != 2:
            ...
            data = resp.json()
            if not data:
                msg = f" obtain historical data is empty "
                self.gateway.write_log(msg)
            else:
                for d in data["data"]:
    ...
    def on_query_account(self, data, request):
        """"""
        if self.check_error(data, " check account "):
            return
    ...
    def on_query_position(self, data, request):
        """"""
        if self.check_error(data, " queries positions "):
            return
    ...
    def on_query_active_order(self, data, request):
        """"""
        if self.check_error(data, " commissioned query activity "):
            return
        ...
        self.gateway.write_log(f"{request.extra} events request information query succeeds ")

    def on_query_history_order(self, data, request):
        """"""
        if self.check_error(data, " historical inquiry commission "):
            return
        ...
        self.gateway.write_log(f"{request.extra} historical information inquiry commissioned successfully ")

    def on_query_trade(self, data, request):
        """"""
        if self.check_error(data, " query deal "):
            return
        ...
        self.gateway.write_log(f"{request.extra} transaction information query success ")
    ...
    def on_send_order(self, data, request):
        ...
        if self.check_error(data, " entrust "):
    ...
    def on_send_order_failed(self, status_code: str, request: Request):
        ...
        msg = f" commissioned failure ， status code ：{status_code}， information ：{request.response.text}"
        self.gateway.write_log(msg)
    ...
    def on_cancel_order(self, data, request):
        """"""
        self.check_error(data, " withdrawals ")

    def on_cancel_order_failed(self, status_code: str, request: Request):
        """
        Callback when canceling order failed on server.
        """
        msg = f" withdrawals failure ， status code ：{status_code}， information ：{request.response.text}"
        self.gateway.write_log(msg)
    ...
    def on_send_orders(self, data, request):
        ...
        if errors:
            for d in errors:
                ...
                msg = f" batch commissioned failure ， status code ：{code}， information ：{msg}"
                self.gateway.write_log(msg)
    ...
    def on_send_orders_failed(self, status_code: str, request: Request):
        ...
        msg = f" batch commissioned failure ， status code ：{status_code}， information ：{request.response.text}"
        self.gateway.write_log(msg)
    ...
    def check_error(self, data: dict, func: str = ""):
        ...
        self.gateway.write_log(f"{func} request error ， code ：{error_code}， information ：{error_msg}")
        return True
...
class HbdmTradeWebsocketApi(HbdmWebsocketApiBase):
    ...
    def on_connected(self):
        """"""
        self.gateway.write_log(" transaction Websocket API connection succeeded ")
        self.login()

    def on_login(self):
        """"""
        self.gateway.write_log(" transaction Websocket API login successful ")
        self.subscribe()
...
class HbdmDataWebsocketApi(HbdmWebsocketApiBase):
    ...
    def on_connected(self):
        """"""
        self.gateway.write_log(" price Websocket API connection succeeded ")
    ...
    def on_data(self, packet):  # type: (dict)->None
        ...
        if channel:
            ...
            msg = packet["err_msg"]
            self.gateway.write_log(f" error code ：{code},  error messages ：{msg}")
...
def _split_url(url):
    """
     will url split into host with path
    :return: host, path
    """
...
def create_signature(api_key, method, host, path, secret_key, get_params=None):
    """
     create a signature 
    :param get_params: dict  use GET additional parameters comes when the method (urlparams)
    :return:
    """
    sorted_params = [
        ("AccessKeyId", api_key),
