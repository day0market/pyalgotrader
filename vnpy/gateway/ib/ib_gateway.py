...
class IbGateway(BaseGateway):
    ...
    def connect(self, setting: dict):
        ...
        """
        host = setting["TWS address "]
        port = setting["TWS port "]
        clientid = setting[" client number "]
...
class IbApi(EWrapper):
    ...
    def connectAck(self):  # pylint: disable=invalid-name
        ...
        """
        self.status = True
        self.gateway.write_log("IB TWS connection succeeded ")
    ...
    def connectionClosed(self):  # pylint: disable=invalid-name
        ...
        """
        self.status = False
        self.gateway.write_log("IB TWS disconnect ")
    ...
    def error(
        ...
        msg = f" information notice ， code ：{errorCode}， content : {errorString}"
        self.gateway.write_log(msg)
    ...
    def updatePortfolio(  # pylint: disable=invalid-name
        ...
        if not exchange:
            msg = f" unsupported exchange positions {contract.conId} {contract.exchange} {contract.primaryExchange}"
            self.gateway.write_log(msg)
            return
    ...
    def subscribe(self, req: SubscribeRequest):
        ...
        if req.exchange not in EXCHANGE_VT2IB:
            self.gateway.write_log(f" it does not support exchange {req.exchange}")
            return
    ...
    def send_order(self, req: OrderRequest):
        ...
        if req.type not in ORDERTYPE_VT2IB:
            self.gateway.write_log(f" the price type is not supported ：{req.type}")
            return ""
