...
class OkexRestApi(RestClient):
    ...
    def on_query_account(self, data, request):
        ...
        self.gateway.write_log(" query successful account funds ")
    ...
    def on_send_order(self, data, request):
        ...
        if error_msg:
            ...
            self.gateway.write_log(f" commissioned failure ：{error_msg}")
    ...
    def on_failed(self, status_code: int, request: Request):
        ...
        """
        msg = f" request failed ， status code ：{status_code}， information ：{request.response.text}"
        self.gateway.write_log(msg)
...
class OkexWebsocketApi(WebsocketClient):
    ...
    def on_packet(self, packet: dict):
        ...
        if "event" in packet:
            ...
            if event == "subscribe":
                ...
                msg = packet["message"]
                self.gateway.write_log(f"Websocket API request an exception ：{msg}")
            elif event == "login":
    ...
    def on_login(self, data: dict):
        ...
        if success:
            ...
            self.gateway.write_log("Websocket API login failed ")
