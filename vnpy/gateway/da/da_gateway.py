...
class DaGateway(BaseGateway):
    ...
    def connect(self, setting: dict):
        ...
        future_address = setting[" transaction server "]
        market_address = setting[" quotes server "]
...
class DaMarketApi(MarketApi):
    ...
    def subscribe(self, req: SubscribeRequest):
        ...
        if self.login_status:
            ...
            if not da_exchange:
                self.gateway.write_log(f" it does not support exchange {req.exchange.value}")
                return
...
class DaFutureApi(FutureApi):
    ...
    def onRspUserLogin(self, error: dict, reqid: int, last: bool):
        ...
        if not error["ErrorID"]:
            ...
            #  inquiries can be traded contracts 
    ...
    def onRspQryTrade(self, data: dict, error: dict, reqid: int, last: bool):
        ...
        if last:
            self.gateway.write_log(" transaction information query success ")
    ...
    def onRspQryCapital(self, data: dict, error: dict, reqid: int, last: bool):
        ...
        if last:
            self.gateway.write_log(" financial information query success ")
    ...
    def onRspQryTotalPosition(self, data: dict, error: dict, reqid: int, last: bool):
        ...
        if last:
            self.gateway.write_log(" position information query success ")
