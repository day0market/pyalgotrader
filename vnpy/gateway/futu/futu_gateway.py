...
class FutuGateway(BaseGateway):
    ...
    default_setting = {
        ...
        " port ": 11111,
        " market ": ["HK", "US"],
        " surroundings ": [TrdEnv.REAL, TrdEnv.SIMULATE],
    }
    ...
    def connect(self, setting: dict):
        """"""
        self.host = setting[" address "]
        self.port = setting[" port "]
        self.market = setting[" market "]
        self.password = setting[" password "]
        self.env = setting[" surroundings "]
    ...
    def connect_trade(self):
        ...
        if code == RET_OK:
            self.write_log(" transaction interface unlock success ")
        else:
            self.write_log(f" unlock failed transaction interface ， the reason ：{data}")
    ...
    def subscribe(self, req: SubscribeRequest):
        ...
        for data_type in ["QUOTE", "ORDER_BOOK"]:
            ...
            if code:
                self.write_log(f" subscribe to market failure ：{data}")
    ...
    def send_order(self, req: OrderRequest):
        ...
        if code:
            self.write_log(f" commissioned failure ：{data}")
            return ""
    ...
    def cancel_order(self, req: CancelRequest):
        ...
        if code:
            self.write_log(f" withdrawals failure ：{data}")
    ...
    def query_contract(self):
        ...
        for product, futu_product in PRODUCT_VT2FUTU.items():
            ...
            if code:
                self.write_log(f" query failed contract information ：{data}")
                return
    ...
    def query_account(self):
        ...
        if code:
            self.write_log(f" query account funds fail ：{data}")
            return
    ...
    def query_position(self):
        ...
        if code:
            self.write_log(f" queries positions fail ：{data}")
            return
    ...
    def query_order(self):
        ...
        if code:
            self.write_log(f" inquiry commission failed ：{data}")
            return

        self.process_order(data)
        self.write_log(" successfully commissioned inquiry ")
    ...
    def query_trade(self):
        ...
        if code:
            self.write_log(f" queries deal fell through ：{data}")
            return

        self.process_deal(data)
        self.write_log(" query successful deal ")
