    ...
    def query_history(self, req: HistoryRequest):
        ...
        while True:
            ...
            if resp.status_code // 100 != 2:
                ...
                data = resp.json()
                if not data:
                    msg = f" obtain historical data is empty ， starting time ：{start_time}"
                    break
...
class BitfinexWebsocketApi(WebsocketClient):
    ...
    def on_trade_update(self, data):
        ...
        if name == "ws":
            for l in info:
                self.on_wallet(l)
            self.gateway.write_log(" account funds for success ")
        elif name == "wu":
            self.on_wallet(info)
        elif name == "os":
            for l in info:
                self.on_order(l)
            self.gateway.write_log(" activities entrusted to succeed ")
    ...
    def on_order_error(self, d):
        ...
        orderid = str(data[2])
        if orderid == "None":
            self.gateway.write_log(" withdrawals failure ， delegate does not exist ")
            return
        ...
        self.gateway.write_log(f" single commission refused ：{error_info}")
