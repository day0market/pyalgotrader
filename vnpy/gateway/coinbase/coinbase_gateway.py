...
class CoinbaseWebsocketApi(WebsocketClient):
    ...
    def connect(
        ...
        """"""
        self.gateway.write_log(" start connection ws interface ")
        self.key = key
    ...
    def on_packet(self, packet: dict):
        ...
        if packet['type'] == 'error':
            self.gateway.write_log(
                "Websocket API an error ： %s" % packet['message'])
            self.gateway.write_log(
                "Websocket API the reason given is ： %s" % packet['reason'])
            self.active = False
...
class CoinbaseRestApi(RestClient):
    ...
    def on_query_order(self, data, request):
        ...
        self.gateway.write_log(u' information inquiry commissioned successfully ')
    ...
    def on_send_order_failed(self, status_code: str, request: Request):
        ...
        if request.response.text:
            ...
            error = data["message"]
            msg = f" commissioned failure ， status code ：{status_code}， information ：{error}"
        else:
    ...
    def on_cancel_order_failed(self, status_code: str, request: Request):
        ...
        if request.response.text:
            data = request.response.json()
            error = data["message"]
            msg = f" withdrawals failure ， status code ：{status_code}， information ：{error}"
        else:
            msg = f" withdrawals failure ， status code ：{status_code}"
    ...
    def on_failed(self, status_code: int, request: Request):
        ...
        data = request.response.json()
        error = data['message']
        msg = f" request failed ， status code ：{status_code}, information ：{error}"
        self.gateway.write_log(msg)
